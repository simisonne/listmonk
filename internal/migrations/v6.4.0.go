...
func V6_4_0(db *sqlx.DB, fs stuffbin.FileSystem, ko *koanf.Koanf, lo *log.Logger) error {
	...
	// counts also collapse rapid duplicate opens. An open is a burst duplicate
	// when the same (campaign_id, subscriber_id) pair recorded a later view
	// within 5 seconds of it, and only the latest view of a burst counts.
	// Kept in sync with the burst filter in schema.sql (mat_dashboard_charts
	// views CTE). No rows are touched, only the derived view is redefined,
	// so this is safe to re-run and exports stay raw.
	if _, err := db.Exec(`
